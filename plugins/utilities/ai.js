export const run = {
   usage: ['ذكاء'], // الاسم العربي الرئيسي الذي سيظهر في القائمة (ذكاء)
   hidden: ['ai', 'بوت', 'اسأل', 'gpt'], // الاختصارات المخفية لضمان عمل البوت كبدائل دائماً
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!text) return client.reply(m.chat, Utils.example(isPrefix, command, 'من أنت؟'), m)
         
         // إرسال تفاعل تفاعلي يفيد ببدء المعالجة والتحضير
         client.sendReact(m.chat, '🕒', m.key)
         
         // إرسال السؤال إلى خادم الذكاء الاصطناعي برمجياً
